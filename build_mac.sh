...
if [[ -f "$MODELFILE_PATH" ]]; then
  EXTRA_DATA_ARGS+=(--add-data "$BUNDLED_MODELS_DIR:bundled_models")
  echo -e "${GREEN}✓ 同梱モデルを app/Resources に含めます: $MODEL_DIR${NC}"
else
  echo -e "${YELLOW}⚠ 同梱モデルが見つかりません（任意）: $MODELFILE_PATH${NC}"
  echo -e "${YELLOW}  初回起動時に ollama pull へフォールバックします。${NC}"
fi
