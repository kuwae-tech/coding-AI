...
def ensure_ollama_installed():
    ...
    logger.warning("Ollama 未検出。初回セットアップ（自動）を試行します。")
    ...
    msg = (
        "Ollama が見つかりません。自動セットアップを試しましたが完了できませんでした。\n"
        f"次のURLからインストールしてください: {installer_url}\n"
...
def ensure_ollama_running(retry_seconds: int = 30):
    ...
    for i in range(retry_seconds):
        if ollama_ok():
            logger.info("Ollama 起動待ち: OK (%s秒)", i + 1)
            return True, ""
        time.sleep(1)
...
def ensure_model_ready():
    ...
    # 1) 同梱モデルがあれば create を優先
    if modelfile.exists():
        logger.info("同梱モデルを検出。ollama create を実行します。")
        try:
            p = _run_logged(["ollama", "create", MODEL_NAME, "-f", str(modelfile)], cwd=str(model_dir), timeout=3600)
            if p.returncode != 0:
                return False, f"同梱モデル準備（ollama create）に失敗しました。\nログ保存先: {STARTUP_LOG_PATH}"
        except Exception as e:
            logger.exception("同梱モデル準備失敗")
            return False, f"同梱モデル準備でエラーが発生しました: {e}\nログ保存先: {STARTUP_LOG_PATH}"
    else:
        ...
        try:
            ...
            if p.returncode != 0:
                return False, f"モデル自動取得（ollama pull）に失敗しました。\nログ保存先: {STARTUP_LOG_PATH}"
        except Exception as e:
            logger.exception("モデル自動取得失敗")
            return False, f"モデル自動取得でエラーが発生しました: {e}\nログ保存先: {STARTUP_LOG_PATH}"
...
def stream_ollama(model, messages, temp=0.7):
    resp = requests.post(
        f"{OLLAMA}/api/chat",
        json={"model": model, "messages": messages, "stream": True, "options": {"temperature": temp}},
        stream=True,
        timeout=300,
    )
...
def main():
    ...
    try:
        ...
        webview.create_window("NEXUS PRO", url, width=1300, height=860, min_size=(920, 640), resizable=True)
